...
func sendWOLuser(w http.ResponseWriter, r *http.Request, db *sql.DB) {
        ...
        result := SendMagicPacket(mac, port, user)
        if !result {
                http.Error(w, "Failed to send magic packet", http.StatusInternalServerError)
                fmt.Println("Failed to send magic packet for User:", strings.Replace(user, "\n", "", -1))
                return
        }

        // Success response
        w.WriteHeader(http.StatusOK)
        fmt.Fprintln(w, "Magic packet sent successfully")
        fmt.Println("Magic packet sent successfully:", result)
}
...
func listUsrToMac(w http.ResponseWriter, r *http.Request, db *sql.DB) {
        ...
        for rows.Next() {
                ...
                checkErr(err)
                IdUsrMacList = append(IdUsrMacList, "<tr><td>"+id+"</td><td>"+name+"</td><td>"+mac+"</td><td> <a href=\"/remUsrToMacWithId?id="+id+"&key="+key+"\"> 🗑️ Remove User</a> </td><td> <a href=\"/sendWOLuser?user="+name+"\"> 🌐 Send WOL packet</a> </td></tr>")
        }
...
func sendWOL(w http.ResponseWriter, r *http.Request) {
        ...
        //fmt.Println(mac)
        result := SendMagicPacket(mac, port, "-")
        if !result {
                http.Error(w, "Failed to send magic packet", http.StatusInternalServerError)
                fmt.Println("Failed to send magic packet for MAC:", strings.Replace(mac, "\n", "", -1))
                return
        }
